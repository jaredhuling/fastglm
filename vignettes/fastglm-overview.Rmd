...
Heteroskedasticity-consistent and cluster-robust covariance matrices are available directly:
...
The `vcovHC` and `vcovCL` methods produce results numerically identical to `sandwich::vcovHC()` and `sandwich::vcovCL()` (Zeileis, Köll, and Graham, 2020) to floating-point precision. They work for sparse and `big.matrix` fits as well, since the full design matrix is preserved on the fitted object (as a reference, not copied).
